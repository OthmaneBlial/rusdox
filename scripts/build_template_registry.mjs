...
const signaturePath = path.join(root, "registry/index.sig.json");
const publicKeyPath = path.join(root, "registry/public-key.pem");
const outputPath = path.join(root, "registry/preview.html");
...
const html = renderRegistry(registry, digest, signature.key_id);
if (checkOnly) {
  ...
  console.log(`Template registry is current (${registry.templates.length} signed templates).`);
} else {
  fs.writeFileSync(outputPath, html);
  console.log(`Generated signed template preview with ${registry.templates.length} entries.`);
}
...
function renderRegistry(value, sha256, keyId) {
  const cards = value.templates.map((entry) => {
    ...
    return `<article class="template-card">
      <img src="../${relativeUrl(entry.preview.url, value.base_url)}" alt="${escapeAttribute(entry.preview.alt)}" loading="lazy" />
      ...
      <div class="actions"><a href="../${relativeUrl(entry.files.template.url, value.base_url)}">Word template</a><a href="../${relativeUrl(entry.files.sample_data.url, value.base_url)}">Sample JSON</a><a href="../${relativeUrl(entry.verified_outputs.parity_html.url, value.base_url)}">Parity evidence</a></div></div>
    </article>`;
  }).join("");
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Verified template registry | RusDox</title><meta name="description" content="Signed, hash-verified Word templates for RusDox."><link rel="canonical" href="https://othmaneblial.github.io/rusdox/registry/preview.html"><style>
